...
if len(sys.argv) not in [2, 3] or sys.argv[1] not in OPTIONS:
  ...
  print(" 7. (Working) MaxSAT With Heuristics and Manual Cardinality Restrictions")
  print(" 8. (Working) MaxSAT With Heuristics and PySat Cardinality Restrictions")
  exit()

# Cargamos segun el tipo de resolucion que usaremos (DOUBLE_TILE o LAYERED)
if sys.argv[1] in [SAT_DOUBLE_TILE, SAT_DOUBLE_TILE_PYSAT_CARDINALITY, MAXSAT_DOUBLE_TILE, MAXSAT_DOUBLE_TILE_PYSAT_CARDINALITY, MAXSAT_WITH_HEURISTICS_MANUAL, MAXSAT_WITH_HEURISTICS_PYSAT ]:
...
USING_MAXSAT = sys.argv[1] in [MAXSAT_DOUBLE_TILE, MAXSAT_DOUBLE_TILE_PYSAT_CARDINALITY, MAXSAT_LAYERED_TILES, MAXSAT_WITH_HEURISTICS_MANUAL, MAXSAT_WITH_HEURISTICS_PYSAT]
...
if sys.argv[1] in [SAT_DOUBLE_TILE, SAT_DOUBLE_TILE_PYSAT_CARDINALITY, MAXSAT_DOUBLE_TILE, MAXSAT_DOUBLE_TILE_PYSAT_CARDINALITY, MAXSAT_WITH_HEURISTICS_MANUAL, MAXSAT_WITH_HEURISTICS_PYSAT]:
  maze = M(pysat_cardinality=(sys.argv[1] in [SAT_DOUBLE_TILE_PYSAT_CARDINALITY, MAXSAT_DOUBLE_TILE_PYSAT_CARDINALITY, MAXSAT_WITH_HEURISTICS_PYSAT]))
else:
  maze = M()
...
if USING_MAXSAT:
  if sys.argv[1] in [ MAXSAT_WITH_HEURISTICS_MANUAL, MAXSAT_WITH_HEURISTICS_PYSAT ]:
    maxsat_clauses_with_heuristic(multipurpose_cnf, maze)
  else:
